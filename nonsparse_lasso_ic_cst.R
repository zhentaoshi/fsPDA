...
mainfunc = function(t, t1, dgp, cst){
  ...
  res = lasso_ic(t = t, t1 = t1, y = y, Y = Y, ic = "WIC", cst = cst)
  ...
  return( c(prd_res, bias, rmse, k) )
}

cst_list = c(0.5, 1, 2, 4, 8, 16)
...
for (dgp in c("iid", "nnd"))
{
  for (cst in cst_list)
  {
    ...
    save (A1, A2, A3, A4, file = paste ("las_ic_", dgp, "_", cst, ".Rda", sep = ""))
  }
}
...
for (dgp in c("iid", "nnd"))
{
  for (cst in cst_list)
  {
    load(paste ("las_ic_", dgp, "_", cst, ".Rda", sep = ""))
    sh = 7 # number of delta dgp
    hf = 3*sh 
    
    fn = paste("las_ic_", dgp, ".xlsx", sep = "")
    ...
    rownames(output) = c("t1 = t2 = 40","t1 = t2 = 80",
                         "t1 = t2 = 100", "t1 = t2 = 200")
    
    MS = matrix(NA, 4, 3)
    rownames(MS) = c("t1 = t2 = 40","t1 = t2 = 80",
                     "t1 = t2 = 100", "t1 = t2 = 200")
    colnames(MS) = c("p_las", "bias_las", "rmse_las")
