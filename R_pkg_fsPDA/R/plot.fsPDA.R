...
#'
#' @param x An object of class `fsPDA`.
...
plot.fsPDA=function(x,tlab=NULL,ylab=NULL,title=NULL,point=TRUE,legend.position="bottom") {
  ...
  ggdata=rbind(data.frame(date=x$in_sample$date,value=x$in_sample$observation,type="observation"),
               data.frame(date=x$out_of_sample$date,value=x$out_of_sample$observation,type="observation"),
               data.frame(date=c(x$in_sample$date,x$out_of_sample$date[1]),
  ...
  plot=ggplot(ggdata,aes(x=date,y=value,color=type,linetype=type))+
    ...
    theme_bw()+theme(legend.title=element_blank(),legend.position=legend.position)+
    labs(title=title) + xlab(tlab) + ylab(ylab)
  if(point) {
